...
%----------------------------------
% "laenge(Liste,Laenge)" ist ein zweistelliges PrŠdikat, welches die Länge einer Liste ermittelt.
%----------------------------------
...
% "atom_number(AtomNummer,Nummer)" ist ein zweistelliges PrŠdikat, welches Nummern der Form '1' zu 1 umwandelt.
% "mod(Summe,Zahl)" ist ein zweistelliges PrŠdikat für die Modulo-Division.
%----------------------------------

isbn_ok(Eingabe) :-
	
	atom_chars(Eingabe,A), 				% Die Eingabe wird in eine Liste umgewandelt.					
	isbn(A),						% Diese Liste wird durch das Prädikat isbn/1 auf Löngenrestriktion
								% geprüft.
